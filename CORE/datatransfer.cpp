...
*/
    ...
    //while(pos<snapshot.fileSize)
    {
        ...
        if(isSegmentExist==false&&segment.refCnt==1)
        {
            ...
            int goodCloud;
            QJsonArray records;
            bool everyCloudIsOk = EveryCloudIsOk(badClouds);
            if(!everyCloudIsOk)
            {
                //找到一个可以正常访问的云
                for(int i=0;i<cloudNum;++i)
                    if(!badClouds.contains(i))
                    {
                        goodCloud = i;
                        break;
                    }
            ...
            for(int i=0;i<segment.blockNum;++i)
            {
                ...
                //不分配block到当前不能访问的云
                if(badClouds.contains(k))
                {
                    blocks[i].cloudID = goodCloud;
                    //记录信息（待badcloud恢复后，迁移block）
                    QJsonObject record;
                    record.insert("bucketname",bucketname);
                    record.insert("blockid",QString(blocks[i].blockID));
                    record.insert("sourcecloud",clouds[goodCloud].cloudName);
                    record.insert("descloud",clouds[k].cloudName);
...
//检查每个云是否都可以正常访问
bool DataTransfer::EveryCloudIsOk(QList<int> &badClouds)
{
    manger = new QNetworkAccessManager();
    bool ok = true;
    for(int i=0;i<cloudNum;++i)
